...
LATEST_DMG_NAME="${APP_NAME}.dmg"
LATEST_DMG_PATH="$OUT_DIR/$LATEST_DMG_NAME"
...
log "Creating stable latest asset names"
cp -f "$DMG_PATH" "$LATEST_DMG_PATH"
shasum -a 256 "$LATEST_DMG_PATH" > "$LATEST_CHECKSUM_PATH"

{
  ...
  echo "latest_dmg_path=$LATEST_DMG_PATH"
  echo "latest_sha256_path=$LATEST_CHECKSUM_PATH"
} > "$METADATA_PATH"
...
log "Latest DMG: $LATEST_DMG_PATH"
log "Latest SHA256: $LATEST_CHECKSUM_PATH"
